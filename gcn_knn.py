...
all_wrong = 0.
all_count = 0
filenames_x = get_file_name('/home/lmx/chen/graphlets/testset/')
filenames_y = get_file_name('/home/lmx/chen/graphlets/testset/')

f1_truth = open('knn_label_truth_huge_total_mod.txt', 'w')
f2_prediction = open('knn_prediction_huge_total_mod.txt', 'w')
...
for ao in range(len(filenames_x)):
    ...
    for pe in range(ao+1, len(filenames_x)):
        ...
        if False:
            ...
            label1_name = '/home/lmx/chen/graphlets/all_label/' + la1 + '_label.txt'
            label2_name = '/home/lmx/chen/graphlets/all_label/' + la2 + '_label.txt'
            ...
            result_dict[la1].append([la2, float(out)])
            label_dict[la1].append([la2, l])

            result_dict[la2].append([la1, float(out)])
            label_dict[la2].append([la1, l])
    ...
    print(len(result_dict[la1]))
    result_dict[la1].sort(key=lambda x:x[1])
    label_dict[la1].sort(key=lambda x:x[1])

    print(la1, file=f1_truth, end='\t')
    print(la1, file=f2_prediction, end='\t')

    for i in range(20):
        print(result_dict[la1][i][0], file=f2_prediction, end='\t')
        print(label_dict[la1][i][0], file=f1_truth, end='\t') 
    print('\n', file=f2_prediction, end='')
    print('\n', file=f1_truth, end='') 
